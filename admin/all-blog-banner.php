    ...
    <div class="panel panel-primary">
      <div class="panel-heading">
        ...
        <div class="col-md-3 text-right">
          <a href="add-blog-banner.php" class="btn btn-sm btn btn-primary"><i class="fa fa-plus-circle"></i> Add  blog Banner</a>
        </div>
      ...
      <div class="panel-body">
        <table class="table table-responsive table-striped table-hover table_cus">
          ...
          <tbody>
              ...
              <?php
               $sel="SELECT * FROM blog_banner ORDER BY blog_ban_id DESC";
               $Q=mysqli_query($con,$sel);
               while ($data=mysqli_fetch_assoc($Q)) {

               ?>
              <tr>
                    <td><?= $data['blog_ban_title']; ?></td>
                    <td class="hidden-xs">
                      <?php
                      if(!empty($data['blog_ban_photo'])){
                        ?>
                        <img height="50" src="uploads/<?=$data['blog_ban_photo'];?>" alt="photo"/>
                        <?php
                      }else{
                        ?>
