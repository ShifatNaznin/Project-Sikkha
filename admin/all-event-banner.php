    ...
    <div class="panel panel-primary">
      <div class="panel-heading">
        ...
        <div class="col-md-3 text-right">
          <a href="add-event-banner.php" class="btn btn-sm btn btn-primary"><i class="fa fa-plus-circle"></i> Add  Event Banner</a>
        </div>
      ...
      <div class="panel-body">
        <table class="table table-responsive table-striped table-hover table_cus">
          ...
          <tbody>
            ...
            $sel="SELECT * FROM event_banner ORDER BY event_ban_id DESC";
            $Q=mysqli_query($con,$sel);
            while ($data=mysqli_fetch_assoc($Q)) {
             ?>
              <tr>
                    <td><?= $data['event_ban_title']; ?></td>
                    <td class="hidden-xs">
                      <?php
                      if(!empty($data['event_ban_photo'])){
                        ?>
                        <img height="50" src="uploads/<?=$data['event_ban_photo'];?>" alt="photo"/>
                        <?php
                      }else{
                        ?>
